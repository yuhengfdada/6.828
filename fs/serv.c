...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if ((r = file_read(o->o_file, (void *)(ret->ret_buf), req->req_n, o->o_fd->fd_offset)) < 0)
		// panic("serve_read: file_read problem\n");
		return r;
	o->o_fd->fd_offset += r;
	return r;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	if ((r = openfile_lookup(envid, req->req_fileid, &o)) < 0)
		return r;
	// req_n should be less than PGSIZE.
	req->req_n = req->req_n > PGSIZE ? PGSIZE : req->req_n;
	if ((req->req_n + o->o_fd->fd_offset) > o->o_file->f_size)
		file_set_size(o->o_file,req->req_n + o->o_fd->fd_offset+1);
